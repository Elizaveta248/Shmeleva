...
class Vacancy:
    ...
    def __init__(self, vacancy):
        ...
        self.salary_average = self.rub[self.salary_currency] * (self.salary_from + self.salary_to) / 2
...
class DataSet:
    ...
    def reading_csv(self):
        with open(self.file_name, mode='r', encoding='utf-8-sig') as file:
            ...
            for r in read:
                if '' not in r and len(r) == header_length:
    ...
    def get_statis(self):
        ...
        for vacancy_dictionary in self.reading_csv():
            vacancy = Vacancy(vacancy_dictionary)
            self.inc(salar, vacancy.year, [vacancy.salary_average])
            if vacancy.name.find(self.vacancy_name) != -1:
                self.inc(salary_of_vac_name, vacancy.year, [vacancy.salary_average])
            self.inc(salary_c, vacancy.area_name, [vacancy.salary_average])
            count_of_vac += 1

        vacancies_num = dict([(key, len(value)) for key, value in salar.items()])
        vacancies_num_by_name = dict([(key, len(value)) for key, value in salary_of_vac_name.items()])

        if not salary_of_vac_name:
            salary_of_vac_name = dict([(key, [0]) for key, value in salar.items()])
            vacancies_num_by_name = dict([(key, 0) for key, value in vacancies_num.items()])

        stats = self.average(salar)
        stats2 = self.average(salary_of_vac_name)
        stats3 = self.average(salary_c)
        stats4 = {}
        for year, salaries in salary_c.items():
            stats4[year] = round(len(salaries) / count_of_vac, 4)
        ...
        return stats, vacancies_num, stats2, vacancies_num_by_name, stats3, stats5
...
class InputConnect:
    def __init__(self):
        ...
        stats1, stats2, stats3, stats4, stats5, stats6 = dataset.get_statis()
...
class Report:
    ...
    def generate_excel(self):
        ...
        t = Side(border_style='thin', color='00000000')
    ...
    def generate_image(self):
        ...
        ax3.barh(list([str(a).replace(' ', '\n').replace('-', '-\n') for a in reversed(list(self.stats5.keys()))]), list(reversed(list(self.stats5.values()))), height=0.5, align='center')
        ...
        ax4.pie([other] + list(self.stats6.values()), labels=['Другие'] + list(self.stats6.keys()),
